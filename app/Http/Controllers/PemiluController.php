...
class PemiluController extends Controller
{
    ...
    public function l21pemilu_store(Request $request, $jenis_event)
    {
        ...
        try {
            if ($Model->save()) {
                $EventTemp = new EventTemp();
                $EventTemp->id = $Model->id; // Set the ID explicitly
                $EventTemp->jenis_event = $Model->jenis_event;
                $EventTemp->ip = $Model->ip;
                $EventTemp->androidid = $Model->androidid;
                $EventTemp->kode = $Model->kode;
                $EventTemp->isklaim = $Model->isklaim;
                $EventTemp->username = $Model->username;
                $EventTemp->hadiah = $Model->hadiah;
                $EventTemp->status = $Model->status;
                $EventTemp->keterangan = $Model->keterangan;
                $EventTemp->prize_id = $Model->prize_id;
                $EventTemp->approve_by = $Model->approve_by;
                $EventTemp->website = $Model->website;
                $EventTemp->save();
    ...
    public function l21pemilu_update(Request $request)
    {
        ...
        if ($affectedRows > 0) {
            EventTemp::where('id', $affectedRows->id)
                ->update([
                    'vote' => $affectedRows->vote
                ]);
            return response()->json(['message' => 'Data berhasil disimpan.'], 201);
        } else {
