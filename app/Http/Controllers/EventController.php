...
class EventController extends Controller
{
    ...
    public function indexproses($jenis_event, $website = 'arwanatoto')
    {
        $datawebsite = Website::get();
        $divisi = auth()->user()->divisi;
        $username = auth()->user()->username;

        $eventsQuery = EventTemp::where('jenis_event', $jenis_event)
            ->where('status', 0)
            ->where('isklaim', 1);
    ...
    public function store(Request $request, $jenis_event)
    {
        ...
        try {
            ...
            $all_request["kode"] = $jenis_event == 2 ? $this->generateKey($jenis_event, $all_request["website"], 6) : 0;

            if (!($jenis_event == 2 || $jenis_event == 1)) {
                $all_request["isklaim"] = 1;
            } else {
                $all_request["isklaim"] = 0;
            }
            ...
            if ($event) {
                EventTemp::create($event->toArray());
            }
    ...
    public function edit($jenis_event, $website, $id)
    {
        ...
        return view('event.update', [
            ...
            'data' => $events,
            'website' => $website,
            'dataprize' => ListPrize::get()->toArray(),
            'jenis_event' => $jenis_event
        ]);
    }
    ...
    public function update(Request $request, $jenis_event, $website)
    {
        try {
            ...
            foreach ($allrequest["id"] as $i => $id) {
                ...
                $eventTemp = EventTemp::where('id', $id)->first();
                if ($eventTemp) {
                    $eventTemp->update([
                        'androidid' => $allrequest["androidid"][$i],
                        'kode' => $allrequest["kode"][$i],
    ...
    public function updateketerangan(Request $request, $jenis_event, $website, $id)
    {
        try {
            ...
            $eventTemp = EventTemp::where('id', $id);
            if ($eventTemp) {
                $eventTemp->update([
                    'keterangan' => $allrequest['keterangan']
                ]);
    ...
    public function changestatus(Request $request, $jenis_event, $website, $status)
    {
        try {
            ...
            $url_spinner = isset($dataToUpdate["url_spinner"]) ? $dataToUpdate["url_spinner"] : '';

            // Update the EventTemp records
            EventTemp::whereIn('id', $id)->update([
                'status' => $status,
                'url_spinner' => $status == 1 ? $url_spinner : null,
            ...
            // Delete the events
            EventTemp::whereIn('id', $id)->delete();
    ...
    public function changestatusApproval(Request $request, $jenis_event, $website, $isklaim)
    {
        try {
            ...
            if ($isklaim == 2) {
                ...
                EventTemp::whereIn('id', $ids)->where('jenis_event', $jenis_event)->update([
                    'status' => $isklaim,
                    'approve_by' => auth()->user()->username
                ]);
            } else {
    ...
    public function findkodedestroy(Request $request, $jenis_event, $website, $search = "")
    {
        try {
            ...
            $eventTemp = EventTemp::whereIn('id', $ids)->get();
            if ($eventTemp->isNotEmpty()) {
                foreach ($eventTemp as $item) {
                    EventTemp::where('id', $item->id)->delete();
                }
            }
    ...
    {
        $data = EventTemp::latest()->where('website', $website)->where('jenis_event', $jenis_event)->get();
