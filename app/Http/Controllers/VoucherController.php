...
class VoucherController extends Controller
{
    ...
    {
        $sql = $this->querySqlOld($id, $search);
        $results = DB::select($sql);
    ...
    public function querySqlOld($id, $search = '')
    {
        $filter = "WHERE 1=1";
        $limit = '';
        if ($id != 0) {
        ...
        $sql = "
            SELECT D.username, B.tipe_generate, A.id, A.userklaim, coalesce(B.isdemo,0) as isdemo, B.bo, B.userid as log,
            C.nama AS jenis_voucher, B.keterangan, A.username AS user_kode, A.kode_voucher, A.userklaim AS norek, A.tgl_klaim, A.status_transfer, A.tgl_exp, A.userid AS `log`
            FROM spinner_voucher_old A 
            INNER JOIN spinner_generatevoucher_old B ON A.genvoucherid = B.id
            LEFT JOIN spinner_jenisvoucher C ON A.jenis_voucher = C.index
            LEFT JOIN users D on B.agentid = D.id
            $filter
            ORDER BY A.urutan
    ...
    public function updateStatus(Request $request)
    {
        try {
            ...
            if ($voucher) {
                // Update status_transfer pada Voucher
                $voucher->update([
                    'status_transfer' => $isChecked == 'true' ? '1' : '0'
                ]);

                // Simpan ke VoucherOld
                VoucherOld::create($voucher->toArray());

                // Hapus Voucher
                $voucher->delete();
            } else {
                // Jika Voucher tidak ditemukan, coba cari VoucherOld
                $voucherOld = VoucherOld::where('id', $id)->first();

                if ($voucherOld) {
                    // Update status_transfer pada VoucherOld
                    $voucherOld->update([
                        'status_transfer' => $isChecked == 'true' ? '1' : '0'
                    ]);

                    // Simpan ke Voucher
                    Voucher::create($voucherOld->toArray());

                    // Hapus VoucherOld
                    $voucherOld->delete();
                } else {
                    // Jika tidak ditemukan Voucher atau VoucherOld
            ...
            dd('Update gagal:', $e->getMessage());
            // atau
            // return redirect()->back()->with('error', 'Gagal memperbarui status transfer voucher: ' . $e->getMessage());
        }
    }
