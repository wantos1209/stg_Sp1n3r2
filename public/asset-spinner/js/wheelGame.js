...
class FortuneWheel extends Phaser.Scene{
    ...
    preload(){
        ...
        // loading images
        wheelConfig.sprites.forEach((s)=>{if(s.fileName != null) this.load.image(s.name, "/asset-spinner/" + wheelConfig.assetPath + "png/" + s.fileName);});
        this.load.spritesheet("coinspin", "/asset-spinner/" + wheelConfig.assetPath + "png/CoinSheet.png", { frameWidth: 200, frameHeight: 200});
        ...
        // loading sounds
        this.load.audio('pointer_hit_clip', ['/asset-spinner/audio/pointer_hit.ogg', '/asset-spinner/audio/pointer_hit.mp3' ]);  // this.load.audio('wheel_spin_clip', '/asset-spinner/audio/spin_sound.mp3'); this.load.audio('coins_clip', '/asset-spinner/audio/win_coins.wav');
        this.load.audio('win_clip', ['/asset-spinner/audio/win_sound1.ogg','/asset-spinner/audio/win_sound1.mp3']);
        this.load.audio('zonk_sound',['/asset-spinner/audio/zonk_soundnew.ogg','/asset-spinner/audio/zonk_soundnew.mp3']);

        // backsound music
        this.load.audio('backsound_clip',['/asset-spinner/audio/backsound_sound.ogg', '/asset-spinner/audio/backsound_sound.mp3']);

        // loading bitmap fonts
        this.load.bitmapFont('sectorFont', '/asset-spinner/fonts/' + wheelConfig.fontName +'.png', '/asset-spinner/fonts/'+ wheelConfig.fontName + '.xml');
    }
    ...
    create(){
        ...
        // Setel header kustom
        xhr.setRequestHeader('Authorization', 'Bearer youk1llmyfvcking3x');
                ...
                onComplete: function(tween){                    // function to be executed once the tween has been completed
                    ...
                    this.tweens.add({                           // another tween to rotate a bit in the opposite direction
                        ...
                        onComplete: function(tween) {
                            ...
                            if(l0x005972 == 0){
                                imgElement.src = '/asset-spinner/china/png/img/JACKPOT-01-01.svg';
                            } else if (l0x005972 == 5 || l0x005972 == 11) {
                                imgElement.src = '/asset-spinner/china/png/img/COBALAGI-01.svg';
                            } else {
                                imgElement.src = '/asset-spinner/china/png/img/SALDO-01.svg';
                            }
                            ...
                            const urlParams = new URLSearchParams(window.location.search);
                            const id = urlParams.get('username');
                            const url = '/api/updatevoucher/' + id;
